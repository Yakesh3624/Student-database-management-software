#functions
def delete():
    comp=[name.get(),dob.get(),std.get(),sec.get(),number.get()]
    count=0
    for j in range(len(b)):
        k=[b[j][0] , b[j][1] , b[j][2] , b[j][3] , b[j][7]]
        if k[0] == comp[0]:
            count+=1
        if k[1] == comp[1]:
            count+=1
        ...
        if str(k[4]) == str(comp[4]):
            count+=1
    if count == 5:
        sql='delete from students where Name=%s and DOB=%s and Standard=%s and Section=%s and Number=%s'
        values=[(name.get()),(dob.get()),(std.get()),(sec.get()),(number.get())]
...
heading=Label(win,text="Delete student",font=('',50,'bold'))
heading.place(x=500,y=35)


#label

namel=Label(win, text='Student name', font=('',14,'bold'))
namel.place(x=400,y=180)

dobl=Label(win, text='Date of Birth',font=('',14,'bold'))
dobl.place(x=400,y=230)

stdl=Label(win, text='Standard', font=('',14,'bold'))
stdl.place(x=400,y=280)

secl=Label(win, text='Section', font=('',14,'bold'))
...
std=Entry(win, width=50)
std.place(x=600,y=285)

sec=Entry(win, width=50)
sec.place(x=600,y=335)
...
#button
select=Button(win, text='Delete student',font=('',14,'bold'),padx=4,pady=3,command=delete)
